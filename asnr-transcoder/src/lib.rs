...
pub trait Decoder<'a, I: AsBytes + Debug + 'a> {
    ...
    ) -> Result<Box<dyn FnMut(I) -> IResult<I, Vec<bool>>>, DecodingError<I>>;
    fn decode_character_string(
        char_string: CharacterString,
    ) -> Result<Box<dyn FnMut(I) -> IResult<I, String>>, DecodingError<I>>;
...
pub trait Encoder<T, O: Extend<T> + Debug + 'static> {
    ...
    fn encode_bit_string(
      bit_string: BitString,
  ) -> Result<Box<dyn FnMut(Vec<bool>, O) -> Result<O, EncodingError>>, EncodingError>;
}
